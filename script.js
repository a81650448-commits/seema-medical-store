...
async function submitOrder(e){
 ...
 try{
  ...
  const items=cart.map(x=>({name:products[x.i][1],pack:products[x.i][2],qty:x.q,price:Number(products[x.i][3])||0})),orderId="SMS-"+Date.now().toString().slice(-8),orderTotal=total();
  ...
  const orderResult=await db.from("orders").insert(payload);if(orderResult.error)throw new Error(orderResult.error.message+(orderResult.error.details?" — "+orderResult.error.details:""));
  document.getElementById("orderForm").hidden=true;document.getElementById("orderSuccess").hidden=false;document.getElementById("orderSuccess").innerHTML=`<strong>Order submitted successfully!</strong><br><br>Order ID: <strong>${orderId}</strong><br>Total: <strong>₹${orderTotal}</strong><br>Payment: <strong>${payment==="UPI"?"UPI / QR — UTR recorded":"Cash on Delivery"}</strong><br><br>Keep this Order ID to track your order.<br><br><button type="button" class="primary" onclick="startAnotherOrder()">🛒 Place Another Order</button>`;cart=[];updateCart();
 }catch(error){console.error("ORDER SUBMISSION ERROR",error);alert(error.message||"Unable to submit the order.")}finally{if(button){button.disabled=false;button.textContent="Confirm & Place Order"}}
}
