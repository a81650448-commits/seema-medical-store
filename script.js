...
function openCheckout(){if(!cart.length){alert("Please add at least one product.");return}document.getElementById("cartOverlay").classList.remove("open");document.getElementById("checkoutItems").innerHTML=cart.map(x=>`<div class="summary-line"><span>${products[x.i][1]} × ${x.q}</span><span>₹${(Number(products[x.i][3])||0)*x.q}</span></div>`).join("");document.getElementById("checkoutTotal").textContent="₹"+total();document.getElementById("checkoutOverlay").classList.add("open")}
...
async function submitOrder(e){
 ...
 const button=document.querySelector('#orderForm button[type="submit"]');if(button){button.disabled=true;button.textContent="Submitting..."}
 try{
  ...
  const txn=document.getElementById("txn").value.trim()||null;
  ...
  const payload={order_id:orderId,customer_name:name,phone:phone,address:address,payment_method:payment,transaction_id:txn,items:items,total:total(),status:"Pending"};
  ...
  document.getElementById("orderSuccess").innerHTML=`<strong>Order submitted successfully!</strong><br><br>Order ID: <strong>${orderResult.data.order_id}</strong><br>Total: <strong>₹${orderResult.data.total}</strong><br><br>Keep this Order ID to track your order.`;
  cart=[];updateCart();
 }catch(error){console.error("ORDER SUBMISSION ERROR",error);alert("Order failed: "+(error.message||"Unknown error"))}
 finally{if(button){button.disabled=false;button.textContent="Submit Order"}}
}
