// Seema Medical Store - SINGLE SHARED CART v10
// One cart state only. Cart persistence is loaded ONLY after medicines are loaded.
(function(){
'use strict';
const KEY='seema_cart_shared_v2';

function productIndexById(id){return products.findIndex(p=>String(p?.[4]?.id)===String(id));}
function validProduct(i){return Number.isInteger(Number(i))&&!!products[Number(i)]&&products[Number(i)][4]?.id!=null;}

function saveCart(){
  try{
    const saved=cart.filter(x=>validProduct(x.i)).map(x=>({id:products[x.i][4].id,q:Math.max(1,Number(x.q)||1)}));
    localStorage.setItem(KEY,JSON.stringify(saved));
  }catch(e){console.error('CART SAVE ERROR',e)}
}

function loadCart(){
  try{
    const raw=JSON.parse(localStorage.getItem(KEY)||'[]');
    if(!Array.isArray(raw))return;
    const restored=[];
    raw.forEach(x=>{
      const i=productIndexById(x.id);
      if(i<0)return;
      const stock=Number(products[i][4]?.stock??0);
      if(stock<=0)return;
      const q=Math.min(Math.max(1,Number(x.q)||1),stock);
      restored.push({i,q});
    });
    cart.length=0;
    restored.forEach(x=>cart.push(x));
  }catch(e){console.error('CART LOAD ERROR',e)}
}

function renderCart(){
  let count=0,totalAmount=0;
  const rows=cart.map(x=>{
    if(!validProduct(x.i))return '';
    const p=products[x.i],price=Number(p[3])||0;
    count+=Number(x.q)||0;totalAmount+=price*(Number(x.q)||0);
    return `<div class="cart-row"><div><strong>${p[1]}</strong><div class="small">${p[2]}</div></div><div class="qty"><button type="button" onclick="changeQty(${x.i},-1)">−</button> <span>${x.q}</span> <button type="button" onclick="changeQty(${x.i},1)">+</button></div><strong>₹${(price*x.q).toFixed(2)}</strong></div>`;
  }).join('');
  const countEl=document.getElementById('cartCount');if(countEl)countEl.textContent=count;
  const totalEl=document.getElementById('cartTotal');if(totalEl)totalEl.textContent='₹'+totalAmount.toFixed(2);
  const items=document.getElementById('cartItems');if(items)items.innerHTML=rows||"<p class='muted'>Your cart is empty.</p>";
  const more=document.getElementById('cartAddMoreButton');if(more)more.hidden=cart.length===0;
  saveCart();
}

window.addToCart=function(i){
  i=Number(i);if(!validProduct(i))return;
  const stock=Number(products[i][4]?.stock??0);
  if(stock<=0){alert('This medicine is currently out of stock.');return;}
  const existing=cart.find(x=>Number(x.i)===i);
  if(existing){
    if(existing.q>=stock){alert('Only '+stock+' unit(s) available.');return;}
    existing.q+=1;
  }else{
    // IMPORTANT: append; never replace the existing cart.
    cart.push({i:i,q:1});
  }
  renderCart();
};

window.changeQty=function(i,d){
  i=Number(i);const existing=cart.find(x=>Number(x.i)===i);if(!existing)return;
  const stock=Number(products[i]?.[4]?.stock??0);
  if(d>0&&existing.q>=stock){alert('Only '+stock+' unit(s) available.');return;}
  existing.q+=Number(d);
  if(existing.q<=0)cart.splice(cart.indexOf(existing),1);
  renderCart();
};
window.updateCart=renderCart;
window.total=function(){return cart.reduce((s,x)=>s+(Number(products[x.i]?.[3])||0)*x.q,0)};
window.addMoreMedicines=function(){
  document.getElementById('cartOverlay')?.classList.remove('open');
  document.getElementById('products')?.scrollIntoView({behavior:'smooth',block:'start'});
};

// This event fires only AFTER products have loaded, so the saved cart is never
// erased while the products array is still empty.
window.addEventListener('medicinesReady',function(){loadCart();renderCart();});

window.addEventListener('beforeunload',saveCart);
})();