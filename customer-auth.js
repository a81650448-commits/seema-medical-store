...
function persistForUser(id){if(!id)return;const saved=currentShared();write(userKey(id),saved)}
function restoreForUser(id){
 if(!id)return;
 const key=userKey(id),saved=read(key);
 if(saved.length)write(CART_KEY,saved);else clearShared();
 // Rebuild the in-memory cart only after medicines have loaded.
...
async function loginCustomer(ev){
 ev.preventDefault();if(!c){msg('Customer login is not configured yet.',true);return}
 msg('Signing in...');
 ...
 restoreForUser(data.user?.id);closeCustomerAuth();refreshCustomerUI();
}
async function signupCustomer(ev){
 ...
 if(error){msg(error.message,true);return}
 if(data.session){restoreForUser(data.user?.id);closeCustomerAuth();refreshCustomerUI()}else msg('Account created. Please check your email. The verification link will return you to Seema Medical Store.')
}
async function logoutCustomer(){
 if(!c)return;
 const{data}=await c.auth.getSession(),id=data.session?.user?.id;
 if(id)persistForUser(id);
 clearShared();
...
async function init(){
 ...
 window.addEventListener('cartUpdated',async()=>{try{const{data}=await c.auth.getSession();const id=data.session?.user?.id;if(id)persistForUser(id)}catch(err){console.error('CUSTOMER CART SYNC ERROR',err)}});
 await refreshCustomerUI();
 c.auth.onAuthStateChange((event,session)=>setTimeout(()=>{refreshCustomerUI();if(event==='SIGNED_IN'&&session?.user)restoreForUser(session.user.id)},0));
}
